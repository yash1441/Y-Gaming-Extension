...
function getElementsFromPage() {
	...
	try {
		...
		eles = Array.from(
			document.getElementsByTagName("main")[0].children[0].children[3]
				.children[0].children[1].children
		);
		// Condition, type, price, etc. selectors over the shop
		document
			.getElementsByTagName("main")[0]
			.children[0].children[3].children[0].children[0].addEventListener(
				"click",
				function () {
					checkIfPageLoaded(300);
				}
		...
		document
			.getElementsByTagName("main")[0]
			.children[0].children[3].children[0].children[2].children[1].children[0].addEventListener(
				"click",
				function () {
					checkIfPageLoaded(300);
				}
	...
	for (let item of itemCards) {
		...
		let searchString = encodeURI(text)
			.replace("%20()%20(FOR%20PREMIUM)", "")
		...
		if (
			url.length > 50 &&
			item.lastChild.className !== "xplay_steam_addon_link"
		) {
			// Add "Check Steam Market" button
			let button = document.createElement("div");
			button.className = "xplay_steam_addon_link";
			button.innerHTML =
				"<a href='" + url + "' target='_blank'>Check Steam Market</a>";
			button.style.cssText =
				"display: inline-block; padding: 10px 15px 10px 15px; margin: 20px 10px 0 0; background-color: #282d32; border-radius: 20px; text-align: center;";
			...
			button2.style.cssText =
				"display: inline-block; padding: 10px 15px 10px 15px; margin-top: 20px; background-color: #282d32; border-radius: 20px; text-align: center;";
			item.style.height = "auto";
			...
			button2.addEventListener("click", function (button2) {
				...
				chrome.runtime.sendMessage({ url: url }, (res) => {
					...
					priceTag.innerHTML = res.results[0].sell_price_text;
					let xcoinRatio = (
						(res.results[0].sell_price / item.getAttribute("cost")) *
						10
					).toFixed(2);
					priceTag.innerHTML +=
						" <small>(" + xcoinRatio + "&hairsp;/&hairsp;1k)</small>";
...
function createNotification(type, title, message) {
	...
	notif.style.cssText =
		"position: absolute; bottom: 20px; right: 35px; background-color: #282d32; border-radius: 5px; width: 300px; min-height: 60px;";
	notifTime.style.cssText =
		"position: absolute; top: 0; left: 0; width: 300px; height: 5px; background-color: " +
		typeColor +
		"; border-radius: 5px 5px 5px 0; transition: width " +
		duration +
		"ms linear;";
	notifTitle.style.cssText =
		"padding: 0; margin: 10px 0 3px 10px; font-size: 22px; color: white;";
	notifMessage.style.cssText =
		"padding: 0 0 10px 0; margin: 0 0 0 10px; font-size: 14px; color: white;";
